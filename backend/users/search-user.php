  ...
  if($_SERVER["REQUEST_METHOD"] == "POST") {
    $email = mysqli_real_escape_string($db,$_POST['email']);
		...
		if ($email == ""){
			...
			$sql="SELECT *, users.nome AS n1, permissions.nome as n1 FROM users,permissions where email='". $email."' && valid=1 && users.permission_id = permissions.permissions_id";
			...
			$res = mysqli_fetch_assoc($result_users );
						...
						echo '  <table id="myTable" class="table" id="myTable" style="width: 100%;">
						  ...
						  <tr>
							...
							<td scope="row" class="nome'. $res['users_id'].'" > '. $res['n1'].' </td>
